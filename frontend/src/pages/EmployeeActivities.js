...
const EmployeeActivities = () => {
 ...
 const barChartCanvas = actions.map((label, i) => ({
   type: 'rect',
   x: 50,
   y: i * 30,
 ...
 const legend = actions.map((label, i) => [
   { text: '', fillColor: colors[i % colors.length], width: 15, height: 15, margin: [0, 0, 5, 0] },
   { text: reverseArabicWords(label), margin: [5, 0, 0, 0], alignment: 'right' },
   { text: `${counts[i]}`, alignment: 'left' }
 ]);
 const docDefinition = {
   defaultStyle: { font: 'Amiri', fontSize: 10 },
   content: [
     ...
     { canvas: [{ type: 'line', x1: 0, y1: 0, x2: 500, y2: 0, lineWidth: 2, lineColor: '#D4AF37' }], margin: [0, 5, 0, 10] },
     { text: 'ملخص الأنشطة', style: 'chartTitle' },
   ...
   styles: {
     header: { fontSize: 22, bold: true, alignment: 'center', margin: [0, 0, 0, 8], color: '#0A1F44' },
     subheader: { fontSize: 10, alignment: 'center', margin: [0, 0, 0, 10] },
     chartTitle: { fontSize: 13, alignment: 'left', bold: true },
     tableHeader: { bold: true, fontSize: 11, color: 'white', fillColor: '#0A1F44', alignment: 'center' },
